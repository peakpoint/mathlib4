...
/-!
### Relations, seen as `Rel α α`
-/

variable {α : Type ua} {β : Type ub} {γ : Type uc} {δ : Type ud} {ι : Sort*}

/-- The identity relation, or the graph of the identity function -/
@[deprecated Rel.id (since := "2025-07-06")]
def idRel {α : Type*} :=
  { p : α × α | p.1 = p.2 }

set_option linter.deprecated false in
@[deprecated Rel.mem_id (since := "2025-07-06")]
theorem mem_idRel {a b : α} : (a, b) ∈ @idRel α ↔ a = b :=
  Iff.rfl

set_option linter.deprecated false in
@[deprecated Rel.id_subset_iff (since := "2025-07-06")]
theorem idRel_subset {s : Rel α α} : idRel ⊆ s ↔ ∀ a, (a, a) ∈ s := by
  simp [subset_def, mem_idRel]

set_option linter.deprecated false in
@[deprecated Rel.exists_eq_singleton_of_prod_subset_id (since := "2025-07-06")]
theorem eq_singleton_left_of_prod_subset_idRel {X : Type*} {S T : Set X} (hS : S.Nonempty)
    (hT : T.Nonempty) (h_diag : S ×ˢ T ⊆ Rel.id) : ∃ x, S = {x} := by
  obtain ⟨x, hx, -⟩ := Rel.exists_eq_singleton_of_prod_subset_id hS hT h_diag; exact ⟨x, hx⟩

set_option linter.deprecated false in
@[deprecated Rel.exists_eq_singleton_of_prod_subset_id (since := "2025-07-06")]
theorem eq_singleton_right_prod_subset_idRel {X : Type*} {S T : Set X} (hS : S.Nonempty)
    (hT : T.Nonempty) (h_diag : S ×ˢ T ⊆ Rel.id) : ∃ x, T = {x} := by
  obtain ⟨x, -, hx⟩ := Rel.exists_eq_singleton_of_prod_subset_id hS hT h_diag; exact ⟨x, hx⟩

@[deprecated (since := "2025-07-06")]
alias eq_singleton_prod_subset_idRel := Rel.exists_eq_singleton_of_prod_subset_id

set_option linter.deprecated false in
/-- The composition of relations -/
@[deprecated Rel.comp (since := "2025-07-06")]
def compRel (r₁ r₂ : Rel α α) :=
  { p : α × α | ∃ z : α, (p.1, z) ∈ r₁ ∧ (z, p.2) ∈ r₂ }

open scoped Rel

set_option linter.deprecated false in
@[deprecated Rel.mem_comp (since := "2025-07-06")]
theorem mem_compRel {α : Type u} {r₁ r₂ : Rel α α} {x y : α} :
    (x, y) ∈ r₁ ○ r₂ ↔ ∃ z, (x, z) ∈ r₁ ∧ (z, y) ∈ r₂ :=
  Iff.rfl

set_option linter.deprecated false in
@[deprecated Rel.inv_id (since := "2025-07-06")]
theorem swap_idRel : Prod.swap '' idRel = @idRel α :=
  Set.ext fun ⟨a, b⟩ => by simpa [image_swap_eq_preimage_swap] using eq_comm

set_option linter.deprecated false in
@[deprecated Monotone.relComp (since := "2025-07-06")]
theorem Monotone.compRel [Preorder β] {f g : β → Rel α α} (hf : Monotone f) (hg : Monotone g) :
    Monotone fun x => f x ○ g x := fun _ _ h _ ⟨z, h₁, h₂⟩ => ⟨z, hf h h₁, hg h h₂⟩

@[deprecated (since := "2025-07-06")] alias compRel_mono := Rel.comp_subset_comp
@[deprecated (since := "2025-07-06")] alias compRel_left_mono := Rel.comp_subset_comp_left
@[deprecated (since := "2025-07-06")] alias compRel_right_mono := Rel.comp_subset_comp_right
@[deprecated (since := "2025-07-06")] alias prodMk_mem_compRel := Rel.prodMk_mem_comp
@[deprecated (since := "2025-03-10")] alias prod_mk_mem_compRel := Rel.prodMk_mem_comp

set_option linter.deprecated false in
@[deprecated Rel.id_comp (since := "2025-07-06")]
theorem id_compRel {r : Rel α α} : idRel ○ r = r :=
  Rel.id_comp _

set_option linter.deprecated false in
@[deprecated Rel.comp_assoc (since := "2025-07-06")]
theorem compRel_assoc {r s t : Rel α α} : r ○ s ○ t = r ○ (s ○ t) := by
  apply Rel.comp_assoc

set_option linter.deprecated false in
@[deprecated Rel.left_subset_comp (since := "2025-07-06")]
theorem left_subset_compRel {s t : Rel α α} (h : idRel ⊆ t) : s ⊆ s ○ t := fun ⟨_x, y⟩ xy_in =>
  ⟨y, xy_in, h <| rfl⟩

set_option linter.deprecated false in
@[deprecated Rel.right_subset_comp (since := "2025-07-06")]
theorem right_subset_compRel {s t : Rel α α} (h : idRel ⊆ s) : t ⊆ s ○ t := fun ⟨x, _y⟩ xy_in =>
  ⟨x, h <| rfl, xy_in⟩

set_option linter.deprecated false in
@[deprecated Rel.left_subset_comp (since := "2025-07-06")]
theorem subset_comp_self {s : Rel α α} (h : idRel ⊆ s) : s ⊆ s ○ s :=
  left_subset_compRel h

set_option linter.deprecated false in
@[deprecated Rel.subset_iterate_comp (since := "2025-07-06")]
theorem subset_iterate_compRel {s t : Rel α α} (h : idRel ⊆ s) (n : ℕ) :
    t ⊆ (s ○ ·)^[n] t := by
...
@[deprecated Rel.IsSymm (since := "2025-07-06")]
def IsSymmetricRel (V : Rel α α) : Prop :=
  Prod.swap ⁻¹' V = V
...
@[deprecated Rel.symmetrize (since := "2025-03-05")]
def symmetrizeRel (V : Rel α α) : Rel α α :=
  V ∩ Prod.swap ⁻¹' V

set_option linter.deprecated false in
@[deprecated Rel.isSymm_symmetrize (since := "2025-03-05")]
theorem symmetric_symmetrizeRel (V : Rel α α) : IsSymmetricRel (symmetrizeRel V) := by
  simp [IsSymmetricRel, symmetrizeRel, preimage_inter, inter_comm, ← preimage_comp]

set_option linter.deprecated false in
@[deprecated Rel.symmetrize_subset_self (since := "2025-03-05")]
theorem symmetrizeRel_subset_self (V : Rel α α) : symmetrizeRel V ⊆ V :=
  sep_subset _ _

set_option linter.deprecated false in
@[deprecated Rel.symmetrize_mono (since := "2025-03-05")]
theorem symmetrize_mono {V W : Rel α α} (h : V ⊆ W) : symmetrizeRel V ⊆ symmetrizeRel W :=
  inter_subset_inter h <| preimage_mono h

set_option linter.deprecated false in
@[deprecated Rel.comm (since := "2025-03-05")]
theorem IsSymmetricRel.mk_mem_comm {V : Rel α α} (hV : IsSymmetricRel V) {x y : α} :
    (x, y) ∈ V ↔ (y, x) ∈ V :=
...
set_option linter.deprecated false in
@[deprecated Rel.inv_eq_self (since := "2025-03-05")]
theorem IsSymmetricRel.eq {U : Rel α α} (hU : IsSymmetricRel U) : Prod.swap ⁻¹' U = U :=
  hU
...
set_option linter.deprecated false in
@[deprecated Rel.isSymm_inter (since := "2025-03-05")]
theorem IsSymmetricRel.inter {U V : Rel α α} (hU : IsSymmetricRel U) (hV : IsSymmetricRel V) :
...
set_option linter.deprecated false in
@[deprecated Rel.isSymm_iInter (since := "2025-03-05")]
theorem IsSymmetricRel.iInter {U : (i : ι) → Rel α α} (hU : ∀ i, IsSymmetricRel (U i)) :
    IsSymmetricRel (⋂ i, U i) := by
  simp_rw [IsSymmetricRel, preimage_iInter, (hU _).eq]

set_option linter.deprecated false in
@[deprecated Rel.IsSymm.sInter (since := "2025-03-05")]
lemma IsSymmetricRel.sInter {s : Set (Rel α α)} (h : ∀ i ∈ s, IsSymmetricRel i) :
    IsSymmetricRel (⋂₀ s) := by
  rw [sInter_eq_iInter]
  exact IsSymmetricRel.iInter (by simpa)

set_option linter.deprecated false in
@[deprecated Rel.isSymm_preimage (since := "2025-03-05")]
...
protected theorem UniformSpace.Core.comp_mem_uniformity_sets {c : Core α} {s : Rel α α}
...
def UniformSpace.Core.mk' {α : Type u} (U : Filter (α × α)) (refl : ∀ r ∈ U, ∀ (x), (x, x) ∈ r)
  ...
  uniformity := U
  refl _r ru := Rel.id_subset_iff.2 ⟨refl _ ru⟩
  symm
...
def UniformSpace.Core.mkOfBasis {α : Type u} (B : FilterBasis (α × α))
  ...
  uniformity := B.filter
  refl := B.hasBasis.ge_iff.mpr fun _r ru => Rel.id_subset_iff.2 ⟨refl _ ru⟩
...
lemma UniformSpace.mem_uniformity_ofCore_iff {u : UniformSpace.Core α} {s : Rel α α} :
...
theorem refl_le_uniformity : 𝓟 Rel.id ≤ 𝓤 α :=
...
theorem refl_mem_uniformity {x : α} {s : Rel α α} (h : s ∈ 𝓤 α) : (x, x) ∈ s :=
  refl_le_uniformity h rfl

theorem mem_uniformity_of_eq {x y : α} {s : Rel α α} (h : s ∈ 𝓤 α) (hx : x = y) : (x, y) ∈ s :=
  refl_le_uniformity h hx

theorem symm_le_uniformity : map (@Prod.swap α α) (𝓤 _) ≤ 𝓤 _ :=
  UniformSpace.symm

theorem comp_le_uniformity : ((𝓤 α).lift' fun s : Rel α α => s ○ s) ≤ 𝓤 α :=
  UniformSpace.comp

theorem lift'_comp_uniformity : ((𝓤 α).lift' fun s : Rel α α => s ○ s) = 𝓤 α :=
  comp_le_uniformity.antisymm <| le_lift'.2 fun s hs ↦ mem_of_superset hs <|
    have : Rel.IsRefl s := ⟨fun _ ↦ refl_mem_uniformity hs⟩; Rel.left_subset_comp

theorem tendsto_swap_uniformity : Tendsto (@Prod.swap α α) (𝓤 α) (𝓤 α) :=
  symm_le_uniformity

theorem comp_mem_uniformity_sets {s : Rel α α} (hs : s ∈ 𝓤 α) : ∃ t ∈ 𝓤 α, t ○ t ⊆ s :=
...
theorem symm_of_uniformity {s : Rel α α} (hs : s ∈ 𝓤 α) :
...
theorem comp_symm_of_uniformity {s : Rel α α} (hs : s ∈ 𝓤 α) :
...
theorem symmetrize_mem_uniformity {V : Rel α α} (h : V ∈ 𝓤 α) : Rel.symmetrize V ∈ 𝓤 α := by
...
theorem UniformSpace.hasBasis_symmetric :
    (𝓤 α).HasBasis (fun s : Rel α α => s ∈ 𝓤 α ∧ Rel.IsSymm s) id :=
  hasBasis_self.2 fun t t_in =>
    ⟨Rel.symmetrize t, symmetrize_mem_uniformity t_in, inferInstance, Rel.symmetrize_subset_self⟩

theorem uniformity_lift_le_swap {g : Rel α α → Filter β} {f : Filter β} (hg : Monotone g)
...
theorem uniformity_lift_le_comp {f : Rel α α → Filter β} (h : Monotone f) :
    ((𝓤 α).lift fun s => f (s ○ s)) ≤ (𝓤 α).lift f :=
  calc
    ((𝓤 α).lift fun s => f (s ○ s)) = ((𝓤 α).lift' fun s : Rel α α => s ○ s).lift f := by
      rw [lift_lift'_assoc]
      · exact monotone_id.relComp monotone_id
      · exact h
    _ ≤ (𝓤 α).lift f := lift_mono comp_le_uniformity le_rfl

theorem comp3_mem_uniformity {s : Rel α α} (hs : s ∈ 𝓤 α) : ∃ t ∈ 𝓤 α, t ○ (t ○ t) ⊆ s :=
  let ⟨_t', ht', ht's⟩ := comp_mem_uniformity_sets hs
  let ⟨t, ht, htt'⟩ := comp_mem_uniformity_sets ht'
  have : Rel.IsRefl t := Rel.id_subset_iff.1 <| refl_le_uniformity ht
  ⟨t, ht, (Rel.comp_subset_comp (Rel.left_subset_comp.trans htt') htt').trans ht's⟩

/-- See also `comp3_mem_uniformity`. -/
theorem comp_le_uniformity3 : ((𝓤 α).lift' fun s : Rel α α => s ○ (s ○ s)) ≤ 𝓤 α := fun _ h =>
  let ⟨_t, htU, ht⟩ := comp3_mem_uniformity h
  mem_of_superset (mem_lift' htU) ht

/-- See also `comp_open_symm_mem_uniformity_sets`. -/
theorem comp_symm_mem_uniformity_sets {s : Rel α α} (hs : s ∈ 𝓤 α) :
    ∃ t ∈ 𝓤 α, Rel.IsSymm t ∧ t ○ t ⊆ s := by
  obtain ⟨w, w_in, w_sub⟩ : ∃ w ∈ 𝓤 α, w ○ w ⊆ s := comp_mem_uniformity_sets hs
  use Rel.symmetrize w, symmetrize_mem_uniformity w_in, inferInstance
  have : Rel.symmetrize w ⊆ w := Rel.symmetrize_subset_self
  calc Rel.symmetrize w ○ Rel.symmetrize w
    _ ⊆ w ○ w := by gcongr
    _ ⊆ s     := w_sub

theorem subset_comp_self_of_mem_uniformity {s : Rel α α} (h : s ∈ 𝓤 α) : s ⊆ s ○ s :=
  have : Rel.IsRefl s := Rel.id_subset_iff.1 <| refl_le_uniformity h; Rel.left_subset_comp

theorem comp_comp_symm_mem_uniformity_sets {s : Rel α α} (hs : s ∈ 𝓤 α) :
    ∃ t ∈ 𝓤 α, Rel.IsSymm t ∧ t ○ t ○ t ⊆ s := by
...
theorem mem_ball_symmetry {V : Rel β β} [V.IsSymm] {x y} : x ∈ ball y V ↔ y ∈ ball x V := Rel.comm V

theorem ball_eq_of_symmetry {V : Rel β β} [V.IsSymm] {x} : ball x V = { y | (y, x) ∈ V } := by
  ext y
  rw [mem_ball_symmetry]
  exact Iff.rfl

theorem mem_comp_of_mem_ball {V W : Rel β β} {x y z : β} [V.IsSymm] (hx : x ∈ ball z V)
    (hy : y ∈ ball z W) : (x, y) ∈ V ○ W := by
  rw [mem_ball_symmetry] at hx
  exact ⟨z, hx, hy⟩

theorem mem_comp_comp {V W M : Rel β β} [W.IsSymm] {p : β × β} :
...
theorem nhds_basis_uniformity' {p : ι → Prop} {s : ι → Rel α α} (h : (𝓤 α).HasBasis p s)
...
theorem UniformSpace.ball_mem_nhds (x : α) ⦃V : Rel α α⦄ (V_in : V ∈ 𝓤 α) : ball x V ∈ 𝓝 x := by
  rw [UniformSpace.mem_nhds_iff]
  exact ⟨V, V_in, Subset.rfl⟩

theorem UniformSpace.ball_mem_nhdsWithin {x : α} {S : Set α} ⦃V : Rel α α⦄ (x_in : x ∈ S)
...
theorem UniformSpace.mem_nhds_iff_symm {x : α} {s : Set α} :
    s ∈ 𝓝 x ↔ ∃ V ∈ 𝓤 α, Rel.IsSymm V ∧ ball x V ⊆ s := by
  rw [UniformSpace.mem_nhds_iff]
  constructor
  · rintro ⟨V, V_in, V_sub⟩
    use Rel.symmetrize V, symmetrize_mem_uniformity V_in, inferInstance
    exact Subset.trans (ball_mono Rel.symmetrize_subset_self x) V_sub
  · rintro ⟨V, V_in, _, V_sub⟩
    exact ⟨V, V_in, V_sub⟩

theorem UniformSpace.hasBasis_nhds (x : α) :
    HasBasis (𝓝 x) (fun s : Rel α α => s ∈ 𝓤 α ∧ Rel.IsSymm s) fun s => ball x s :=
  ⟨fun t => by simp [UniformSpace.mem_nhds_iff_symm, and_assoc]⟩

open UniformSpace

theorem UniformSpace.mem_closure_iff_symm_ball {s : Set α} {x} :
    x ∈ closure s ↔ ∀ {V}, V ∈ 𝓤 α → Rel.IsSymm V → (s ∩ ball x V).Nonempty := by
...
theorem mem_nhds_left (x : α) {s : Rel α α} (h : s ∈ 𝓤 α) : { y : α | (x, y) ∈ s } ∈ 𝓝 x :=
  ball_mem_nhds x h

theorem mem_nhds_right (y : α) {s : Rel α α} (h : s ∈ 𝓤 α) : { x : α | (x, y) ∈ s } ∈ 𝓝 y :=
...
theorem lift_nhds_left {x : α} {g : Set α → Filter β} (hg : Monotone g) :
    (𝓝 x).lift g = (𝓤 α).lift fun s : Rel α α => g (ball x s) := by
...
theorem Filter.HasBasis.biInter_biUnion_ball {p : ι → Prop} {U : ι → Rel α α}
...
theorem uniformContinuous_of_const [UniformSpace β] {c : α → β} (h : ∀ a b, c a = c b) :
    UniformContinuous c :=
  have : (fun x : α × α => (c x.fst, c x.snd)) ⁻¹' Rel.id = univ :=
...
theorem Filter.HasBasis.uniformContinuous_iff {ι'} [UniformSpace β] {p : ι → Prop}
    {s : ι → Rel α α} (ha : (𝓤 α).HasBasis p s) {q : ι' → Prop} {t : ι' → Set (β × β)}
